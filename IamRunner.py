...
class AlertExport():
    ...
    def getalerts(self):
        ...
        for policy in policies:
            list_names.append(policy['name'])
        filters.append({'name': 'alert.status', 'value': 'Resolved', 'operator': '='})
        for filter in self.config.rl_filter_resource_type :
        ...
        data = json.dumps({
            ...
            'filters': filters,
            'limit': 10,
            'policy.name':list_names,
            'timeRange': {
                'type': 'relative',
                'value': {
                    'amount': self.config.rl_timerange_amount,
                    'unit': self.config.rl_timerange_unit
                }
            }
        })
